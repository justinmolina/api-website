...
import * as RenderActions from 'store/actions/renderActions'

//helper functions
import {setBackground,MAP_NAMES } from './HelperFuncs'
import { rendererActionType } from 'src/constants/rendererActionType';
...
class Scene extends React.Component {
  ...
  init(){
    ...
    var renArray = this.props.renderer._root.entries[0][1]
    this.renArray= renArray
    var renderer = renArray.pop()
    ...
    //^^ was 4
    // scene.add(cube)
    setBackground(this.props.color, this.state.scene)
  
     renderer.setSize(width, height)
     renderer.physicallyCorrectLights = true;
     renderer.gammaOutput = this.state.gamma || false
    ...
    this.controls.addEventListener('change' , ()=> renderer.render(this.state.scene,this.camera))
 ...
 loadModel = (gltfmodel,scene, camera)=> {
  ...
  manager.onProgress = ( url, itemsLoaded, itemsTotal )=> {
    this.bar.style.width = (itemsLoaded / itemsTotal * 100) + '%';
...
manager.onLoad = ( url, itemsLoaded, itemsTotal )=> {
  ...
  this.bar.innerHTML = ''
  this.mount.style.visibility = 'visible'
  }
  ...
  this.props.AddScn(this.state.scene)
  // 
...
if(isVisible && this.state.hasloaded){
  this.props.AddScn(this.state.scene)
}
if(isVisible && !this.state.hasloaded){
  // this.loadModel(this.props.model,this.state.scene,this.camera);
  
  this.state.scene.name = "inner scene"
...
this.renArray.push(this.renderer)
this.mount.removeChild(this.renderer.domElement)


  // this.props.renderer._root.entries[0][1].push(this.renderer)
}
if(this.state.isVisible && this.state.scene === null){
this.init()
}
}
exposureChange(e){
...
ismobile() {
  ...
  render() {
    ...
    if(!this.state.editMenu) {
      return(
        <VisibilitySensor onChange= {this.onChange} partialVisibility={this.props.partial || false} scrollCheck={true} >
...
const mapDispatchToProps = (dispatch, ownProps) => {
  return {
     AddScn: (scene) => dispatch(RenderActions.addScene(scene)),
     Alert:() => dispatch(RenderActions.alert('HELROO'))
  }
}
...
const mapStateToProps = state =>{
  ...
  const renderer = state.getIn(['renderer'])
  const StateScene = state.getIn(['renderer', 'Scene'])
  const StateCamera = state.getIn(['renderer', 'StateCamera'])
  return{
